...
function series_xy_logk(series, scale, npast, nfuture; decay = 1, concat_valid_map = nothing, localdiff = 0)
    ...
    #
    return series_xy_logk_numba(series, scale, npast, nfuture, decay,
     concat_valid_map, kernel_params_[1], kernel_params_[2], localdiff)
end

function series_xy_logk_numba(series, scale, npast, nfuture, decay, concat_valid_map, kernel_params_1, kernel_params_2, localdiff)
    ...
    #= println()
    println("npast = ", npast)
    println("nfuture = ", nfuture)
    println("factor_r_past = ", factor_r_past)
    println("factor_r_future = ", factor_r_future)
    println("decay = ", decay)
    println("concat_valid_map = ", concat_valid_map)
    println("kernel_params_1 = ", kernel_params_1)
    println("kernel_params_2 = ", kernel_params_2) =#
   ...
   if n%2 == 1  m = n else m = n-1 end
    ...
    for k in prange((n+1)//2,n)

        for l in range(k)

            i=k
            j=l
            sumx, sumy = sxy_logk(i, j, series, concat_valid_map, npast, nfuture, localdiff, kernel_params_2)
            sx[i,j] = sumx
            sx[j,i] = sumx
            sy[i,j] = sumy
            sy[j,i] = sumy
...
function sxy_logk(i, j, series, concat_valid_map, npast, nfuture, localdiff, kernel_params_2)
            
    # from index of (past,future) pairs to index in data series
    i = concat_valid_map[i]
    j = concat_valid_map[j]

    delta = zeros(series.shape[1])
    ...
    for t in range(npast)
        ...
        if kernel_params_2 != 2
            ds = np.power(ds, 0.5*kernel_params_2)
        end
