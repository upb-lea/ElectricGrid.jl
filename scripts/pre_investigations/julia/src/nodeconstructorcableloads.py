...
class NodeConstructorCableLoads:
    ...
    def __init__(
        self, num_source, num_loads, CM=None, parameter=None, S2S_p=0.1, S2L_p=0.8
    ):
        ...
        if isinstance(parameter, dict):
            assert (
                len(list(parameter.keys())) == 3
            ), f"Expect parameter to have three entries but got {len(list(parameter.keys()))}"

            assert sorted(list(parameter.keys())) == sorted(
                ["cable", "source", "load"]
            ), f"Expect parameter to have the three entries 'cable', 'load' and 'source' but got {sorted(list(parameter.keys()))}."

            self.parameter = parameter

            assert self.num_source == len(
            ...
            assert self.num_connections == len(
                self.parameter["cable"]
            ...
            self.num_LCL, self.num_LC, self.num_L = self._cntr_fltr(
                self.parameter["source"]
            )
            ...
            (
                ...
                self.num_loads_C,
                self.num_loads_R,
            ) = self._cntr_loads(self.parameter["load"])

            assert self.num_source == (
                self.num_LCL + self.num_LC + self.num_L
    ...
    def _get_filter_distribution(self):
        ...
        self.num_LC = 1#int(np.ceil(np.clip(sample, 1, self.num_source - 1)))
    ...
    def _get_load_distribution(self):
        ...
        self.num_loads_C = 1#int(np.floor(sample[1]))
        self.num_loadss_L = 1#int(np.floor(sample[2]))
        self.num_loads_RL = 1#int(np.floor(sample[3]))
        self.num_loads_RC = 1#int(np.floor(sample[4]))
        self.num_loads_LC = 1#int(np.floor(sample[5]))
    ...
    def _sample_fltr_LC(self):
        ...
        source = dict()
        source["fltr"] = "LC"
        source["R"] = 0.4 # np.round_(np.random.uniform(0.1, 1), 3)
        source["L1"] = 2.3e-3 # np.round_(np.random.uniform(2, 2.5), 3) * 1e-3
        source["C"] = 10e-6 # np.round_(np.random.uniform(5, 15), 3) * 1e-6
    ...
    def _sample_load_C(self):
        ...
        load = dict()
        load["impedance"] = "C"
        load["C"] = 10e-6 # np.round_(np.random.uniform(1, 10), 3)
        return load
    ...
    def _sample_load_L(self):
        ...
        load = dict()
        load["impedance"] = "L"
        load["L"] = 2.3e-3 # np.round_(np.random.uniform(1, 10), 3)
        return load
    ...
    def _sample_load_RL(self):
        ...
        load = dict()
        load["impedance"] = "RL"
        load["R"] = 0.4 # np.round_(np.random.uniform(10, 10000), 3)
        load["L"] = 2.3e-3 # np.round_(np.random.uniform(1, 10), 3)
        return load
    ...
    def generate_CM(self):
        ...
        # fill matrix with random entries between [0,1]
        CM = np.random.rand(self.tot_ele, self.tot_ele) * mask
        # delet diagonal bc no connection with itself
        CM = CM - np.eye(CM.shape[0]) * np.diag(CM)
        ...
        for i in range(self.tot_ele):
            ...
            non_zero = np.sum([entries[i] != 0 for i in range(len(entries))])
            # number of zero entries
            zero = np.sum([entries[i] == 0 for i in range(len(entries))])
            ...
            if non_zero <= 2:  # we need to set values if there are less then 2 entries
                ...
                # Get rows of the entries = 0
                idx_row_entries = np.where(0 == Col)
                # Get col of the entries = 0
                idx_col_entries = np.where(0 == Row)
    ...
    def generate_A_load_row(self, load_i):
        ...
        if (
            parameter_i["impedance"] == "RLC"
            or parameter_i["impedance"] == "LC"
            or parameter_i["impedance"] == "RL"
            or parameter_i["impedance"] == "L"
        ):
            ...
            for i, (idx, sign) in enumerate(zip(indizes_, signs)):
                ...
                A_load_row[idx - 1, 0] = (
                    sign * 1 / self.parameter["cable"][idx - 1]["L"]
                )

        elif (
            parameter_i["impedance"] == "RC"
            or parameter_i["impedance"] == "C"
            or parameter_i["impedance"] == "R"
        ):
    ...
    def generate_A(self):
        ...
        A_col_list = [self.get_A_col(i) for i in range(1, self.num_source + 1)]
        ...
        # start at 1 bc Source 1 ...
        A_row_list = [self.get_A_row(i) for i in range(1, self.num_source + 1)]
        ...
        # i-> idx // i+1 -> num of load
        A_load_row = np.concatenate((A_load_row_list), axis=1)
        ...
        A_load_diag = np.zeros((self.num_impedance, self.num_impedance))
    ...
    def generate_B(self):
        ...
        """
        B = np.zeros(
            (self.num_fltr + self.num_connections + self.num_impedance, self.num_source)
        )

        # start at 1 bc Source 1 ...
        B_source_list = [self.get_B_source(i) for i in range(1, self.num_source + 1)]
