...
if __name__ == '__main__':
    ...
    plt_setting = dict({'c': [0]})


    for cc in range(len(plt_setting['c'])):

        y_debug_julia = np.array(time_result_julia['y_debug']['DP5_c'+str(cc+1)+'_nodes8_tend0.03'])
        y_debug_python = np.array(time_result_python['y_debug']['scipy_solve_ivp_c'+str(cc)+'_nodes8_tend0.03'])
