...
class NodeConstructorCableLoads:
    ...
    def __init__(
        ...
        if isinstance(parameter, dict):
            ...
            self.num_LCL, self.num_LC, self.num_L = self.cntr_fltr(
                self.parameter["source"]
            )
            ...
            (
                ...
                self.num_loads_RC,
                self.num_loads_R,
            ) = self.cntr_loadtypes(self.parameter["load"])
    ...
    def get_filter_distribution(self):
        ...
        # int(np.ceil(np.clip(sample, 1, self.num_source - 1)))
        self.num_LC = 1
    ...
    def get_load_distribution(self):
        ...
        sample = np.random.dirichlet(np.ones(4)) * self.num_loads

        # TODO
        self.num_loads_R = self.num_loads  # int(np.floor(sample[0]))
        self.num_loads_RL = 0  # int(np.floor(sample[1]))
        self.num_loads_RC = 0  # int(np.floor(sample[2]))
        # self.num_loads - (self.num_loads_R + self.num_loads_RL + self.num_loads_RC)
        self.num_loads_RLC = 0
        pass
    ...
    def generate_A(self):
        ...
        # get A_load_diag
        self.num_impedance = (
            2 * (self.num_loads_RLC + self.num_loads_RL)
            + self.num_loads_RC
            + self.num_loads_R
        )
        ...
        for i, ele in enumerate(A_load_list):
            if i < self.num_loads_RLC:
                start = 2 * i
                stop = 2 * i + 2
                A_load_diag[start:stop, start:stop] = ele

            elif i < self.num_loads_RLC + self.num_loads_RL:
                start = 2 * i
                stop = 2 * i + 2
                A_load_diag[start:stop, start:stop] = ele

            elif i < self.num_loads_RLC + self.num_loads_RL + self.num_loads_RC:
                start = i + self.num_loads_RLC + self.num_loads_RL
                stop = i + 1 + self.num_loads_RLC + self.num_loads_RL
                A_load_diag[start:stop, start:stop] = ele
    ...
    def get_states(self):
        ...
        for l in range(1, self.num_loads + 1):
            if l <= self.num_loads_RLC + self.num_loads_RL:
